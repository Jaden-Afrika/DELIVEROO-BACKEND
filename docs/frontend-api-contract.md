...
- Env var: `VITE_API_URL` (for local development, use `http://127.0.0.1:8001`)
...
| GET | `/parcels/<id>/tracking` | — | current tracking details and coordinates |
...
| `estimatedTravelTime` | integer | Minutes from the route when available; otherwise estimated from distance |
...
The backend geocodes parcel addresses with OpenStreetMap's Nominatim service and
calculates driving routes with OSRM. These enrichments are best-effort, so parcel
creation and destination updates still succeed if either public service is
unavailable. No Google Maps server-side API key is required.
...
2. **Price recalculation on destination change:** Yes, distance and price are recalculated using OpenStreetMap geocoding/routing services. Documented decision.
3. **`estimatedTravelTime` unit:** Integer minutes, using OSRM duration when available and a 40 km/h estimate as fallback. Documented decision.
